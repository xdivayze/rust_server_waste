        ...
        let handle = {
            ...
            let handle = tokio::task::spawn(async move {
                ...
                let master_waste = Waste {
                    ...
                    id,
                    image_path: (*path.clone()).parse().unwrap(),
                    number: format!("{}00", count).parse().unwrap(),
                    probability: 0.0,
                };
                split_picture_horizontally(SPLIT_COUNT, &master_waste, &mut stream).await.unwrap();
                let mut data = object! {
                ...
                data.insert("children", children).unwrap();
